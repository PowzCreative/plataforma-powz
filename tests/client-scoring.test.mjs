...
import { classifyClient, buildTavilyQueries, normalizeClient } from '../api/lib/client-scoring.mjs';

test('classifies a founder explicitly seeking a Meta Ads freelancer as a hot client', () => {
  const text = "I'm the founder of an ecommerce brand and need a freelance Meta Ads expert to manage and scale our campaigns.";
...
test('keeps a traditional Media Buyer job as a lower-intent client result', () => {
  const text = 'We are hiring a full-time Media Buyer to join our marketing team.';
  const result = classifyClient(text);
  assert.equal(result.isClient, true);
...
test('builds worldwide and Dominican Republic search queries', () => {
  ...
  assert.ok(local.some(q => /Dominican Republic|Santo Domingo|República Dominicana/i.test(q)));
});
